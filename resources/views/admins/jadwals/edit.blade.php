...
@section('title', 'Edit Jadwal - Admin Dashboard')
...
<div class="p-4 mx-auto max-w-screen-2xl md:p-6">
    ...
    <form action="{{ route('jadwals.update', $jadwal->jadwal_id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                ...
                <div class="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">Jadwal Saat Ini:</p>
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div class="flex items-center gap-2">
                            <i class="fa-solid fa-route text-blue-500"></i>
                            <span class="text-sm font-medium text-gray-800 dark:text-white">{{ $jadwal->rute->departure }} - {{ $jadwal->rute->destination }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <i class="fa-solid fa-ship text-brand-500"></i>
                            <span class="text-sm font-medium text-gray-800 dark:text-white">{{ $jadwal->ship->ship_name }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <i class="fa-solid fa-calendar text-gray-500"></i>
                            <span class="text-sm font-medium text-gray-800 dark:text-white">{{ \Carbon\Carbon::parse($jadwal->departure_date)->format('d M Y') }} - {{ $jadwal->departure_time }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="relative">
                        ...
                        <select name="rute_id" id="rute_id" 
                            ...
                            @foreach($rutes as $rute)
                                <option value="{{ $rute->rute_id }}" {{ old('rute_id', $jadwal->rute_id) == $rute->rute_id ? 'selected' : '' }}>
                    ...
                    <div class="relative">
                        ...
                        <select name="ship_id" id="ship_id" 
                            ...
                            @foreach($ships as $ship)
                                <option value="{{ $ship->ship_id }}" data-capacity="{{ $ship->capacity }}" {{ old('ship_id', $jadwal->ship_id) == $ship->ship_id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input
                                ...
                                name="departure_date"
                                id="departure_date"
                                value="{{ old('departure_date', $jadwal->departure_date ? \Carbon\Carbon::parse($jadwal->departure_date)->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input
                                type="time"
                                name="departure_time"
                                value="{{ old('departure_time', $jadwal->departure_time) }}"
                                step="60"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input
                                ...
                                name="seats_kuota"
                                id="seats_kuota"
                                value="{{ old('seats_kuota', $jadwal->seats_kuota) }}"
                                placeholder="Maks: {{ $jadwal->ship->capacity }}"
                                min="0"
                                max="{{ $jadwal->ship->capacity }}"
                        ...
                        @enderror
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            Kuota kursi tersedia: {{ $jadwal->seats_kuota }} dari {{ $jadwal->ship->capacity }} kursi
                        </p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="status" 
                            ...
                            <option value="departed" {{ old('status', $jadwal->status) == 'departed' ? 'selected' : '' }}>Berangkat</option>
                            <option value="arrived" {{ old('status', $jadwal->status) == 'arrived' ? 'selected' : '' }}>Tiba</option>
                ...
                <!-- Warning if seats sold -->
                @if(($jadwal->ship->capacity ?? 0) - ($jadwal->seats_kuota ?? 0) > 0)
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-warning-700 dark:text-warning-300">
                                Sudah ada {{ ($jadwal->ship->capacity ?? 0) - ($jadwal->seats_kuota ?? 0) }} tiket yang terjual untuk jadwal ini. 
                                Perubahan pada jadwal dapat mempengaruhi penumpang yang sudah membeli tiket.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-gray-700 pt-6">
                    <a href="{{ route('jadwals.show', $jadwal->jadwal_id) }}" class="px-4 py-2.5 text-sm font-medium text-gray-700 transition rounded-lg bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
                        Batal
                    </a>
...
@push('scripts')
<script>
    // Update seats quota max based on selected ship
    document.getElementById('ship_id').addEventListener('change', function() {
        ...
        if (capacity) {
            ...
            // Reset value if it exceeds new capacity
