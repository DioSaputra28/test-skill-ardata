    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="font-medium text-primary">Rute</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
    ...
    <div class="space-y-5 sm:space-y-6">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex justify-between items-center">
                ...
                <a href="{{ route('rutes.create') }}" class="inline-flex items-center gap-2 px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">
                                ...
                                <tr>
                                    ...
                                    <td class="px-5 py-4 sm:px-6">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('rutes.show', $rute->rute_id) }}" class="rounded-md bg-blue-500/10 px-3 py-1 text-xs font-medium text-blue-500 hover:bg-blue-500/20">
                                                View
                                            </a>
                                            <a href="{{ route('rutes.edit', $rute->rute_id) }}" class="rounded-md bg-warning-500/10 px-3 py-1 text-xs font-medium text-warning-500 hover:bg-warning-500/20">
                                                Edit
                                            </a>
                                            <form action="{{ route('rutes.destroy', $rute->rute_id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this route?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="5" class="px-5 py-8 text-center">
                                        <div class="flex flex-col items-center">
                                            <i class="fa-solid fa-road text-4xl text-gray-300 dark:text-gray-600 mb-3"></i>
                                            <p class="text-gray-500 dark:text-gray-400">No routes found</p>
                                            <a href="{{ route('rutes.create') }}" class="mt-3 text-primary hover:underline">Add your first route</a>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
